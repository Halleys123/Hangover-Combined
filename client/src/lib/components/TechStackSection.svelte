	...
	const techStack = [
		{
			...
			description: "High-performance asynchronous backend engine powering our REST API, handling heavy PDF buffer ingestion and multi-stage text processing.",
			color: "#339933",
		...
		},
		{
			...
			description: "Enforces strict end-to-end type safety across client and server. Guarantees deterministic schema healing and prevents runtime data corruption.",
			color: "#3178C6",
		...
		},
		{
			...
			description: "Powering our ultra-reactive UI with Svelte 5 runes ($state, $derived), sleek dark mode glassmorphism, and instant routing without page reloads.",
			color: "#FF3E00",
		...
		},
		{
			...
			description: "Our core AI memory graph! Constructs relational knowledge graphs from raw PDF datasheets, enabling zero-hallucination DRC checks and memory recall.",
			color: "#A855F7",
		...
		},
		{
			...
			description: "Our robust document database. Persists user accounts, project metadata, and interactive visual canvas coordinates (nodes and edges).",
			color: "#47A248",
			textColor: "text-[#47A248]",
		...
		},
		{
			...
			description: "Renders the interactive node-based schematic canvas. Manages drag-and-drop IC positioning, custom hardware nodes, and animated wiring.",
			color: "#FF0072",
		...
		},
		{
			...
			description: "Lightweight and robust backend routing framework. Orchestrates PDF upload endpoints, multer file buffering, and Cognee cloud graph synchronization.",
			color: "#94A3B8",
		...
		},
		{
			...
			description: "Provides state-of-the-art styling, glassmorphism effects, custom gradients, and fluid micro-animations for an executive-grade user experience.",
			color: "#38BDF8",
...
<section id="tech-stack" class="py-24 relative overflow-hidden scroll-mt-20">
	...
	<div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[700px] bg-gradient-to-tr from-indigo-600/15 via-purple-600/10 to-cyan-500/10 rounded-full blur-[140px] pointer-events-none -z-10"></div>
	<div class="absolute bottom-10 left-10 w-[300px] h-[300px] bg-blue-600/10 rounded-full blur-[100px] pointer-events-none -z-10"></div>

	<div class="max-w-7xl mx-auto px-6">
		<!-- SECTION HEADER -->
		<div class="text-center max-w-3xl mx-auto mb-16">
			<div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 text-xs font-mono font-semibold uppercase tracking-widest mb-4 shadow-sm">
			...
			<p class="text-base sm:text-lg text-slate-400 mt-4 leading-relaxed">
				Built with state-of-the-art frameworks and cloud cognitive AI infrastructure to deliver autonomous, zero-hallucination hardware synthesis.
			</p>
		</div>
		...
		>
			...
			<svg class="absolute inset-0 w-full h-full pointer-events-none z-10" viewBox="0 0 800 550" preserveAspectRatio="none">
				<defs>
					{#each techStack as tech, i}
						<linearGradient id="wire-grad-{i}" x1="50%" y1="50%" x2="0%" y2="0%">
							<stop offset="0%" stop-color="#6366F1" stop-opacity="0.8" />
							<stop offset="100%" stop-color={tech.color} stop-opacity="1" />
						</linearGradient>
					{/each}
				</defs>
				...
				{#each techStack as tech, i}
					...
					<path
						...
						stroke-width="2"
						fill="none"
						class="transition-colors duration-300 {activeTech === i ? 'stroke-slate-600' : ''}"
					/>
					...
					<path
						...
						class="transition-opacity duration-500 {activeTech === i ? 'opacity-100' : 'opacity-40'}"
					>
						<animate
					...
					<path
						...
						stroke-width="3"
						fill="none"
						class="transition-opacity duration-300 {activeTech === i ? 'opacity-90 drop-shadow-[0_0_8px_currentColor]' : 'opacity-0'}"
					/>
				{/each}
			</svg>
			...
			<div class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-20 flex flex-col items-center">
				<div class="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl bg-gradient-to-tr from-blue-600 via-indigo-600 to-purple-600 p-0.5 shadow-2xl shadow-indigo-500/40 animate-pulse">
					<div class="w-full h-full bg-[#0F1423] rounded-[14px] flex flex-col items-center justify-center p-2 text-center group-hover/diagram:bg-[#0B0F19] transition-colors">
						<div class="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-tr from-blue-500 to-indigo-500 flex items-center justify-center text-white font-black text-xl sm:text-2xl shadow-inner">
							H
						</div>
						<span class="text-[10px] sm:text-[11px] font-mono font-bold text-indigo-300 mt-1 tracking-tight">Hangover</span>
					</div>
				</div>
				<span class="mt-2 text-[10px] font-mono uppercase tracking-widest text-indigo-400 bg-indigo-950/80 px-2.5 py-0.5 rounded-full border border-indigo-800/60 shadow-sm">
					Copilot Hub
				</span>
			</div>
			...
			{#each techStack as tech, i}
				<button
					...
					class="absolute z-30 flex flex-col items-center group transition-all duration-300 -translate-x-1/2 -translate-y-1/2 {activeTech === i ? 'scale-110 sm:scale-125 z-40' : 'hover:scale-105 opacity-80 hover:opacity-100'}"
					style="left: {nodePositions[i].left}; top: {nodePositions[i].top};"
					aria-label={`Select ${tech.name}`}
				>
					<div class="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-slate-900/95 border transition-all duration-300 flex items-center justify-center shadow-lg backdrop-blur-md p-2.5 sm:p-3 {activeTech === i ? `${tech.borderColor} shadow-lg shadow-indigo-500/30 bg-slate-800/90 ring-2 ring-indigo-500/30` : 'border-slate-700/80 group-hover:border-slate-500'}">
						{@html tech.icon}
					</div>
					<span class="mt-1.5 text-[10px] sm:text-xs font-semibold px-2.5 py-0.5 rounded-full bg-slate-900/90 border transition-all duration-300 whitespace-nowrap shadow-md {activeTech === i ? `text-white ${tech.borderColor} bg-slate-800 font-bold` : 'text-slate-400 border-slate-800 group-hover:text-slate-200'}">
						{tech.name}
					</span>
				</button>
			{/each}
		...
		<div class="mt-8 max-w-4xl mx-auto bg-gradient-to-r from-slate-900/90 via-[#131A2B]/90 to-slate-900/90 border border-slate-800/80 rounded-2xl p-6 sm:p-8 shadow-2xl backdrop-blur-xl transition-all duration-300 flex flex-col sm:flex-row items-center gap-6 relative overflow-hidden">
			<!-- Subtle Accent Glow -->
			<div class="absolute top-0 left-0 w-2 h-full {techStack[activeTech].bgColor}" style="background-color: {techStack[activeTech].color};"></div>

			<!-- Icon Box -->
			<div class="w-16 h-16 rounded-2xl flex items-center justify-center shrink-0 bg-slate-900 border border-slate-700/80 shadow-lg p-3 transition-transform duration-300 scale-105">
			...
			<div class="flex-1 text-center sm:text-left">
				<div class="flex flex-wrap items-center justify-center sm:justify-start gap-2.5 mb-2">
					<span class="text-[11px] font-mono font-bold px-2.5 py-0.5 rounded-md uppercase tracking-wider bg-slate-800 text-indigo-300 border border-slate-700">
			...
			<!-- Status Indicator -->
			<div class="shrink-0 flex items-center gap-2.5 px-3 py-1.5 rounded-xl bg-slate-900/80 border border-slate-800 text-xs font-mono text-slate-400">
				<span class="w-2 h-2 rounded-full {isHovered ? 'bg-amber-400' : 'bg-emerald-400 animate-pulse'}"></span>
